...
页面支持简体中文和英文切换，右下角浮动的 `Language / 语言` 控件会把选择保存到浏览器 `localStorage` 的 `ccvar.locale`。Web 版、macOS 客户端和 Windows 客户端使用同一套嵌入式 UI，因此三端共用同一套翻译。弹窗打开时该浮动控件会自动隐藏，避免遮挡表单和按钮。
