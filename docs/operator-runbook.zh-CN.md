...
普通用户下载包在 GitHub Releases：

- `https://github.com/ccvar/quantlab/releases`
- `ccvar-quant-lab-macos-native.zip`：完整 macOS 客户端，包含 `CCVar Quant Lab.app` 和 `CCVar Quant Lab Web.app`。
- `ccvar-quant-lab-windows-native.zip`：完整 Windows 客户端，包含 `CCVar Quant Lab.exe` 和 `Start CCVar Quant Lab Web.cmd`。
- `ccvar-quant-lab-macos-arm64.zip`、`ccvar-quant-lab-windows-amd64.zip`：便携浏览器启动器包。
- `SHA256SUMS.*.txt` 和 `release-manifest.json`：下载校验材料。

## GitHub Actions 构建

仓库包含 `.github/workflows/build-clients.yml`、`.github/workflows/native-desktop-clients.yml` 和 `.github/workflows/publish-release.yml`，会在 push、pull request、`v*` tag 和手动触发时运行：
...
- `Publish Release`：在 `v*` tag 或手动触发时构建浏览器启动器和原生桌面包，并创建/更新 GitHub Release 下载资产。
...
页面支持简体中文和英文切换，语言入口位于左侧品牌区版本号后方，选择会保存到浏览器 `localStorage` 的 `ccvar.locale`。同一位置也提供深色/浅色主题切换，并把主题保存到 `ccvar.theme`。Web 版、macOS 客户端和 Windows 客户端使用同一套嵌入式 UI，因此三端共用同一套翻译和主题资源。
