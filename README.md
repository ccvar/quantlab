...
The macOS clients store data in `~/Library/Application Support/CCVar Quant Lab/ccvar_quant.db` and logs in `~/Library/Application Support/CCVar Quant Lab/logs/client.log`. The Windows clients store data in `%APPDATA%\CCVar Quant Lab\ccvar_quant.db` and logs in `%APPDATA%\CCVar Quant Lab\logs\client.log`. Standalone native clients prefer `127.0.0.1:8787` and automatically choose the next available loopback port when that default is already occupied; browser launchers still open the default browser. `CCVAR_ADDR` disables that fallback by explicitly choosing the bind address, and `CCVAR_DB_PATH` can override the SQLite path.
